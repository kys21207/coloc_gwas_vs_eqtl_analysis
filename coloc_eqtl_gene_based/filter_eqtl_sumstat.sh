...
#zcat $EQTL_FILE | tail -n +2 | awk 'BEGIN {OFS="\t"} {print $2, $3, $3, $0}' > $TMP_DIR/eqtl.bed

# Use bedtools to intersect the BED files and extract relevant lines
bedtools intersect -a $EQTL_FILE -b $TMP_DIR/significant_ld_block.bed -wa | cut -f4- > $TMP_DIR/filtered_eqtl
